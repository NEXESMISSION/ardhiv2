...
export function SalesRecordsPage() {
  ...
  const [_allSales, setAllSales] = useState<Sale[]>([]) // Store all sales for search
  const itemsPerPage = 20
  // Styled success / error popup state — replaces native window.alert calls
  // throughout the page so the design matches the rest of the app.
  const [salesNotification, setSalesNotification] = useState<{ type: 'success' | 'error'; title: string; message: string } | null>(null)
  ...
  async function handleRevertToPending(salesToRevert?: Sale[]) {
    ...
    try {
      ...
      // Delete installment payments if any — must not silently swallow errors,
      // otherwise orphan rows survive and the customer appears to still owe.
      ...
      // Delete appointments tied to these sales so they don't appear as
      // orphan reminders in the Appointments page after the sale is reverted.
      {
      ...
      setSalesNotification({
        type: 'success',
        title: t('salesRecords.revertSuccessCount').split('{{')[0].trim() || 'تم الإرجاع',
        message: replaceVars(t('salesRecords.revertSuccessCount'), { count: sales.length }),
      })
  ...
  async function handleCancelSale(salesToCancel?: Sale[]) {
    ...
    try {
      ...
      // Delete installment payments if any — surface the error so we don't
      // leave orphan rows that look like outstanding debt to the customer.
      const installmentSales = sales.filter(s => s.payment_method === 'installment')
      if (installmentSales.length > 0) {
        const installmentSaleIds = installmentSales.map(s => s.id)
        const { error: instErr } = await supabase
      ...
      // Delete appointments tied to these sales so they don't appear as
      // orphan reminders in the Appointments page after cancellation.
      {
        const { error: aptErr } = await supabase
          .from('appointments')
          .delete()
      ...
      setSalesNotification({
        type: 'success',
        title: t('salesRecords.cancelSuccessCount').split('{{')[0].trim() || 'تم الإلغاء',
        message: replaceVars(t('salesRecords.cancelSuccessCount'), { count: sales.length }),
      })
      setActionDialogOpen(false)
      setSelectedSale(null)
      setSelectedSales(new Set())
  ...
  async function handleRemoveSale(salesToRemove?: Sale[]) {
    ...
    try {
      ...
      setSalesNotification({
        type: 'success',
        title: t('salesRecords.deleteSuccessCount').split('{{')[0].trim() || 'تم الحذف',
        message: replaceVars(t('salesRecords.deleteSuccessCount'), { count: sales.length }),
      })
      setActionDialogOpen(false)
      setSelectedSale(null)
      setSelectedSales(new Set())
  ...
  function openActionDialog(sale: Sale | null, action: 'revert' | 'cancel' | 'revertFromInstallments' | 'remove', useMultiSelect = false) {
    if (useMultiSelect) {
      ...
      if (selectedArray.length === 0) {
        setSalesNotification({
          type: 'error',
          title: t('salesRecords.selectFirst'),
          message: t('salesRecords.selectFirst'),
        })
        return
      }
      setSelectedSale(null)
    ...
    <div className="px-2 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-6 space-y-3 sm:space-y-4 lg:space-y-6">
      ...
      >
        <div className="space-y-2">
          <Label htmlFor="sales-piece-search" className="text-xs sm:text-sm">{t('installments.pieceNumber')}</Label>
          <Input
            id="sales-piece-search"
            type="text"
      ...
      {/* Single notification popup that handles both success + error states.
          Replaces the four window.alert() calls that used to fire on
          revert / cancel / delete / select-first actions. */}
      <NotificationDialog
        open={!!salesNotification}
        onClose={() => setSalesNotification(null)}
        type={salesNotification?.type ?? 'success'}
