...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Track every loadSystemUser-retry setTimeout so signOut can cancel them.
  // Without this, a slow login → logout → re-login as a different user can
  // end with the previous user's retry firing and overwriting the new session.
  const pendingRetryTimersRef = useRef<Set<ReturnType<typeof setTimeout>>>(new Set())
  // Cross-tab logout coordination. When one tab signs out, the other tabs
  // need to drop their cached user state too — otherwise tab B keeps acting
  // as if logged in until its next API call. BroadcastChannel is supported
  // everywhere modern (https://caniuse.com/broadcastchannel).
  const authBroadcastRef = useRef<BroadcastChannel | null>(null)

  // Wire up the cross-tab broadcast channel once. When another tab signs
  // out, we clear our own React state so the user doesn't keep using a now-
  // invalid session in this tab.
  useEffect(() => {
    if (typeof BroadcastChannel === 'undefined') return
    const channel = new BroadcastChannel('ardhi-auth')
    authBroadcastRef.current = channel
    channel.onmessage = (ev) => {
      if (ev.data?.type !== 'signed_out') return
      // Mirror the local-state side of signOut, but DO NOT call
      // supabase.auth.signOut() again (the originating tab already did it
      // and the session is already cleared from the shared storage).
      setUser(null)
      setSystemUser(null)
      ...
      pendingRetryTimersRef.current.forEach((id) => clearTimeout(id))
      pendingRetryTimersRef.current.clear()
      if (loadTimeoutRef.current) {
        clearTimeout(loadTimeoutRef.current)
        loadTimeoutRef.current = null
      }
      try { sessionStorage.removeItem('previousPage') } catch {}
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (event === 'SIGNED_OUT') {
        ...
        // Cancel any in-flight retry timers so they can't fire after sign-out
        // and overwrite the next user's session.
        pendingRetryTimersRef.current.forEach((id) => clearTimeout(id))
        pendingRetryTimersRef.current.clear()
        // Drop module-level caches that should not survive across users.
        clearContractWritersCache()
        clearAllPieceLocks()
        // Drop per-tab navigation memory so a previous user's last page
        // doesn't briefly flash for the next user.
        try { sessionStorage.removeItem('previousPage') } catch {}
        return
      }
  ...
  // (Previous unused `checkNetworkConnection` ping helper removed — the
  // retry logic relies on the abort-controller / timeout machinery already
  // in place inside loadSystemUser, so the explicit ping was never invoked.)
            ...
            if (abortControllerRef.current === abortController && loadingSystemUserRef.current) {
              ...
              if (retryCount < MAX_RETRIES - 1) {
                ...
                const retryId = setTimeout(() => {
                  pendingRetryTimersRef.current.delete(retryId)
                  loadSystemUser(authUserId, retryCount + 1).catch((e) => { if (DEBUG_AUTH) console.error(e) })
                }, delay)
                pendingRetryTimersRef.current.add(retryId)
              } else {
                setLoading(false)
  ...
  async function signOut() {
    try {
      ...
      // Cancel any in-flight retry timers so they can't fire post-logout
      pendingRetryTimersRef.current.forEach((id) => clearTimeout(id))
      pendingRetryTimersRef.current.clear()
      if (loadTimeoutRef.current) {
        clearTimeout(loadTimeoutRef.current)
      ...
      // Scrub the PWA service-worker caches so the next user on this device
      // can't read the previous user's Supabase responses (NetworkFirst, 24h TTL).
      if ('caches' in window) {
        try {
          const names = await caches.keys()
          await Promise.all(
            names
              .filter((n) => /supabase|workbox|google-fonts/i.test(n))
              .map((n) => caches.delete(n))
          )
        } catch (e) {
          if (DEBUG_AUTH) console.warn('Cache scrub failed (non-critical):', e)
        }
      }

      // Tell every other open tab in this browser to clear its session too.
      try { authBroadcastRef.current?.postMessage({ type: 'signed_out' }) } catch {}
  ...
  // Memoize so consumers don't re-render every time AuthProvider rerenders for unrelated reasons.
  const contextValue = useMemo(() => ({
    ...
    isOwner,
    refreshSystemUser,
  }), [user, systemUser, loading, isOwner])
