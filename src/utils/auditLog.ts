 ...
 *
 * Security: the `userId` / `userEmail` / `userName` arguments are IGNORED for
 * the saved row — we always derive the actor from the live Supabase session.
 * Trusting caller-supplied values would let any worker frame the Owner by
 * passing the Owner's id, destroying audit-trail integrity. The args are
 * kept in the signature only for backward compatibility with existing call
 * sites and the dev-mode console log.
 */
export async function logAuditEvent(
  ...
  entityId: string,
  details: Record<string, any> = {},
  // The remaining args are kept for backward compatibility with existing call
  // sites but are intentionally ignored — actor identity is now derived from
  // the live Supabase session (see resolution code below). Prefixed with `_`
  // so TypeScript doesn't flag them as unused.
  _userId?: string,
  _userEmail?: string,
  _userName?: string,
  ...
  try {
    // Resolve the actor from the current session (auth_user_id → users.id),
    // ignoring any caller-provided id. Best-effort: if any of these lookups
    // fail, we record null rather than a forgeable value.
    let trustedUserId: string | null = null
    let trustedUserEmail: string | null = null
    ...
    try {
      ...
      if (authUserId) {
        const { data: profile } = await supabase
          ...
          .single()
        if (profile) {
          trustedUserId = profile.id ?? null
          trustedUserEmail = profile.email ?? null
          trustedUserName = profile.name ?? null
        }
      }
    } catch {
      // Best-effort; record nulls.
    }
    ...
    const auditEntry: AuditLogEntry = {
      ...
      entity_id: entityId,
      details,
      user_id: trustedUserId ?? undefined,
    ...
    try {
      ...
      // Try to include user email and name from the trusted session lookup.
      if (trustedUserEmail) basicData.user_email = trustedUserEmail
      if (trustedUserName) basicData.user_name = trustedUserName
      ...
      if (basicError) {
        ...
        if (errorMsg.includes('user_email') || errorMsg.includes('user_name') || errorCode === 'PGRST204') {
          ...
          if (minimalError && import.meta.env.DEV) {
