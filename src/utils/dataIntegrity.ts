...
/**
 * Drop every active piece lock. Call on sign-out so a stuck lock from a
 * crashed sale flow doesn't survive into the next user's session on the
 * same device, and so cleanupOrphanedReservations can act on those pieces.
 */
export function clearAllPieceLocks(): void {
  activeOperationLocks.clear()
}
...
export async function cleanupOrphanedReservations(): Promise<number> {
  try {
    ...
    // Pre-filter the candidate set so we only do one round-trip to `sales`
    // instead of one query per piece. Skip locked pieces and recently-updated
    // ones (active reservation grace period) up front.
    const candidatePieces = reservedPieces.filter((piece: any) => {
    ...
    if (candidatePieces.length === 0) return 0

    // Single bulk query: which of these pieces still have a pending sale?
    const candidatePieceIds = candidatePieces.map((p: any) => p.id)
    const { data: pendingSales, error: salesError } = await supabase
      .from('sales')
      .select('land_piece_id')
      .in('land_piece_id', candidatePieceIds)
      .eq('status', 'pending')

    if (salesError) {
      console.error('Error checking pending sales for orphan cleanup:', salesError)
      return 0
    }

    const piecesWithPendingSale = new Set((pendingSales ?? []).map((s: any) => s.land_piece_id))
    const orphanIds = candidatePieceIds.filter((id: string) => !piecesWithPendingSale.has(id))

    if (orphanIds.length === 0) return 0

    // Single bulk update for all orphans (RLS-respecting status guard preserved).
    ...
    if (updateError) {
      console.error('Error cleaning up orphaned reservations (bulk):', updateError)
      return 0
    }

    cleanedCount = updated?.length ?? 0
    if (import.meta.env.DEV && cleanedCount > 0) {
      console.log(`Cleaned up ${cleanedCount} orphaned reservations`)
    }
