...
export function UsersPage() {
  ...
  // Styled error popup state — replaces the native alert() that used to fire
  // on delete failures so the dialog matches the rest of the app.
  ...
  async function loadWorkers() {
    ...
    try {
      const { data, error: err } = await supabase
        .from('users')
        .select('id,email,name,phone,place,title,notes,role,allowed_pages,allowed_batches,allowed_pieces,display_order,image_url,auth_user_id,created_at,updated_at,created_by')
  ...
  async function openCreateDialog() {
    ...
    setFormData({
      ...
      // Default: all operational pages, but NEVER the user-management page — Owner must opt-in.
      allowed_pages: availablePages.map(p => p.id).filter(id => id !== 'users'),
      allowed_batches: availableBatches.map(b => b.id),
      allowed_pieces: [],
      image_url: null,
    })
    setImageFile(null)
  ...
  async function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file) {
      ...
      // Magic-byte check defeats SVG-with-script renamed to .png attempts.
  ...
  async function handleSaveWorker() {
    ...
    if (!editingWorkerId && formData.password.length < 8) {
    ...
    try {
      ...
      if (imageFile) {
        try {
          // Generate unique filename. Scope under the editing worker id (or
          // current user if creating) so paths never collide between users
          // and a malicious actor can't `.remove()` someone else's path by
          // guessing the timestamp. Bucket-level RLS should enforce the prefix.
          const fileExt = imageFile.name.split('.').pop()
          const ownerId = editingWorkerId ?? systemUser?.id ?? 'new'
      ...
      if (editingWorkerId) {
        ...
        if (formData.password.trim()) {
          ...
          if (worker) {
            const { data: pwData, error: pwErr } = await supabase.functions.invoke('admin-users', {
              body: {
                action: 'update_password',
                auth_user_id: worker.auth_user_id ?? undefined,
                email: worker.email,
                password: formData.password.trim(),
              },
            })
            if (pwErr) throw new Error(pwData?.error ?? pwErr.message)
        ...
        // Create auth user via Edge Function (server-side, with Owner-role check)
        const { data: authData, error: authError } = await supabase.functions.invoke('admin-users', {
          body: { action: 'create', email, password },
        })
        if (authError) {
          throw new Error(authData?.error ?? authError.message)
        }
        if (authData?.error) {
        ...
        if (err) {
          // If user creation fails, roll back the auth user
          if (authData?.user?.id) {
            await supabase.functions.invoke('admin-users', {
              body: { action: 'delete', auth_user_id: authData.user.id },
            }).catch(console.error)
          }
          throw err
        }
  ...
  async function handleDeleteWorker() {
    ...
    try {
      ...
      if (userData?.auth_user_id) {
        await supabase.functions.invoke('admin-users', {
          body: { action: 'delete', auth_user_id: userData.auth_user_id },
        }).catch((error) => {
      ...
      setDeleteErrorMessage(t('users.deleteError') + ': ' + e.message)
    } finally {
      setDeleting(false)
    }
  }
...
{t('users.cancel')}
        ...
        {/* Styled error popup — replaces the old window.alert on delete failure */}
        <NotificationDialog
