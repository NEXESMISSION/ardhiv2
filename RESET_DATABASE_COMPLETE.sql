-- ============================================
-- COMPLETE DATABASE RESET - KEEP ONLY ONE USER
-- ============================================
-- This script will:
...
-- 6. Includes messaging system tables (conversations, messages, notifications, worker_profiles)
--
...
-- Disable RLS on messaging system tables
ALTER TABLE IF EXISTS notifications DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS messages DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS conversations DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS worker_profiles DISABLE ROW LEVEL SECURITY;

-- Disable RLS on recurring expenses templates
ALTER TABLE IF EXISTS recurring_expenses_templates DISABLE ROW LEVEL SECURITY;
...
-- Delete notifications (references users, conversations, messages)
DELETE FROM notifications;

-- Delete messages (references conversations, users)
DELETE FROM messages;

-- Delete conversations (references users)
DELETE FROM conversations;

-- Delete worker profiles (references users)
...
-- Delete recurring expenses templates (references expense_categories, users)
DO $$ 
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'recurring_expenses_templates') THEN
    DELETE FROM recurring_expenses_templates;
  END IF;
END $$;
...
-- Re-enable RLS on messaging system tables
ALTER TABLE IF EXISTS notifications ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS messages ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS conversations ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS worker_profiles ENABLE ROW LEVEL SECURITY;

-- Re-enable RLS on recurring expenses templates
ALTER TABLE IF EXISTS recurring_expenses_templates ENABLE ROW LEVEL SECURITY;
...
-- ============================================
-- STEP 6: Reset sequences (optional but recommended)
-- ============================================
-- Note: Most tables use UUID, but if you have any serial/sequence columns, reset them here
...
UNION ALL
SELECT 'Recurring Templates:', COALESCE((SELECT COUNT(*) FROM recurring_expenses_templates), 0)
UNION ALL
...
-- ✅ Messaging system tables cleared (conversations, messages, notifications, worker_profiles)
-- ============================================
