...
export function EditSaleDialog({ open, onClose, sale, onSave, isOwner = true }: EditSaleDialogProps) {
  ...
  // Draft persistence — keyed per sale.id so editing different sales gets
  // different drafts. Survives connection drops + accidental closes; cleared
  // automatically on successful save below.
  type DraftShape = {
    salePrice: string
    depositAmount: string
    paymentMethod: 'full' | 'installment' | 'promise'
  ...
  const draft = useFormDraft<DraftShape>('edit-sale', {
    open,
    key: sale?.id ?? null,
    keepOnClose: true,
  })
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error: err }) => {
        ...
        // After DB values land, restore any saved draft on top so a user who
        // lost connection mid-edit picks up exactly where they left off.
        ...
        if (saved) {
          ...
          if (saved.paymentOfferId) setPaymentOfferId(saved.paymentOfferId)
          if (saved.deadlineDate) setDeadlineDate(saved.deadlineDate)
          if (saved.notes) setNotes(saved.notes)
          if (saved.partialPaymentAmount) setPartialPaymentAmount(saved.partialPaymentAmount)
          if (saved.remainingPaymentAmount) setRemainingPaymentAmount(saved.remainingPaymentAmount)
        }
      })
  ...
  // Persist the form to localStorage on every change so a connection drop or
  // accidental close doesn't lose what the user typed. Cleared on save success.
  useEffect(() => {
    if (!open || loadingSale) return
    draft.write({
      salePrice,
      depositAmount,
      paymentMethod,
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, loadingSale, salePrice, depositAmount, paymentMethod, paymentOfferId, deadlineDate, notes, partialPaymentAmount, remainingPaymentAmount])
  ...
  useEffect(() => {
    if (open && sale) {
      ...
      if (justOpened || saleIdChanged) {
        ...
        // Clear payment offers from any previous sale's batch so the dropdown
        // doesn't briefly show offers belonging to a different batch.
        setPaymentOffers([])
  ...
  async function loadPaymentOffers(batchId: string): Promise<PaymentOffer[]> {
    ...
    try {
      ...
      // If the user has since switched to a different sale, drop this result
      // so we don't overwrite the new sale's offer list with the old batch's.
      if (initedSaleIdRef.current !== requestedSaleId) return []
      ...
      if (initedSaleIdRef.current === requestedSaleId) setLoadingOffers(false)
    }
  }
  ...
  async function handleAddNewOffer() {
    ...
    try {
      const payload = {
        ...
        monthly_amount: newOfferCalcMode === 'monthlyAmount' ? parseAmount(newOfferMonthlyAmount) : null,
  ...
  const installmentPreview = useMemo(() => {
    ...
    const deposit = parseAmount(depositAmount) || 0
    const formPrice = parseAmount(salePrice)
  ...
  async function handleSave() {
    ...
    if (paymentMethod === 'promise') {
      const partial = parseAmount(partialPaymentAmount) || 0
      const remaining = parseAmount(remainingPaymentAmount) || 0
    ...
    try {
      ...
      if (paymentMethod === 'promise') {
        updateData.partial_payment_amount = parseAmount(partialPaymentAmount) || deposit
        updateData.remaining_payment_amount = parseAmount(remainingPaymentAmount) || (price - deposit)
      } else {
      ...
      // Sale updated successfully — clear the draft so it doesn't repopulate
      // next time. Failed saves keep the draft so the user can retry.
      draft.clear()
      onSave(updatedSale)
      onClose()
    } catch (e: any) {
  ...
  useEffect(() => {
    if (paymentMethod === 'promise') {
      const price = parseAmount(salePrice) || 0
      const deposit = parseAmount(depositAmount) || 0
      const partial = parseAmount(partialPaymentAmount) || 0
    ...
    >
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="edit-sale-price" className="text-xs sm:text-sm">
        ...
        <div className="space-y-2">
          ...
          {salePrice && depositAmount && (
            <p className="text-xs text-gray-500">
              {t('editSale.remainingAfterDeposit')}: {formatPrice((parseAmount(salePrice) || 0) - (parseAmount(depositAmount) || 0))} DT
            </p>
          )}
        </div>
        ...
        {/* Payment Method */}
        <div className="space-y-2">
          <Label htmlFor="edit-sale-method" className="text-xs sm:text-sm">{t('editSale.saleTypeLabel')}</Label>
          <select
            id="edit-sale-method"
            value={paymentMethod}
        ...
        {paymentMethod === 'installment' && (
          <div className="space-y-2">
            <Label htmlFor="edit-sale-offer" className="text-xs sm:text-sm">{t('editSale.installmentOfferLabel')}</Label>
            {loadingOffers ? (
                  ...
                  <Card className="p-3 bg-blue-50 border-blue-200 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <Label htmlFor="new-offer-name" className="text-xs">{t('editSale.offerNameLabelShort')}</Label>
                        <Input
                          id="new-offer-name"
                          value={newOfferName}
                      ...
                      <div className="space-y-1">
                        <Label htmlFor="new-offer-price-per-m2" className="text-xs">{t('editSale.pricePerM2Label')}</Label>
                        <Input
                          id="new-offer-price-per-m2"
                          type="number"
                          min="0"
                          step="0.01"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <Label htmlFor="new-offer-advance-mode" className="text-xs">{t('editSale.advanceTypeLabel')}</Label>
                        <select
                          id="new-offer-advance-mode"
...
<option value="fixed">{t('editSale.advanceFixed')}</option>
                      ...
                      <div className="space-y-1">
                        <Label htmlFor="new-offer-advance-value" className="text-xs">{t('editSale.advanceValueLabel')}</Label>
                        <Input
                          id="new-offer-advance-value"
                          type="number"
                          min="0"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="new-offer-calc-mode" className="text-xs">{t('editSale.calcMethodLabel')}</Label>
                      <select
                        id="new-offer-calc-mode"
              ...
              <Card className="p-3 bg-green-50 border-green-200 mt-2">
                ...
                <div className="space-y-1 text-xs sm:text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-blue-600">{formatPrice(parseAmount(depositAmount) || 0)} DT</span>
                  </div>
                  <div className="flex justify-between">
        ...
        {paymentMethod === 'promise' && (
          <div className="space-y-3">
            <Card className="p-3 bg-orange-50 border-orange-200">
              ...
              <div className="space-y-2">
                <div className="space-y-1">
                  <Label htmlFor="edit-sale-partial-payment" className="text-xs sm:text-sm">{t('editSale.amountReceivedLabel')}</Label>
                  <Input
                    id="edit-sale-partial-payment"
                    type="number"
                    min="0"
                    step="0.01"
                ...
                <div className="space-y-1">
                  <Label htmlFor="edit-sale-remaining" className="text-xs sm:text-sm">{t('editSale.amountRemainingLabel')}</Label>
                  <div className="flex items-center gap-2">
                    <Input
                      id="edit-sale-remaining"
                      type="number"
                      min="0"
                      step="0.01"
                    ...
                    <span className="text-xs sm:text-sm font-semibold text-orange-600 whitespace-nowrap">
                      {formatPrice(parseAmount(remainingPaymentAmount) || 0)} DT
                    </span>
                  </div>
                  <p className="text-xs text-gray-500">
                    {t('editSale.remainingAutoUpdate')}: {t('finance.price')} ({formatPrice(parseAmount(salePrice) || 0)}) - {t('finance.deposit')} ({formatPrice(parseAmount(depositAmount) || 0)}) - {t('editSale.receivedLabel')} ({formatPrice(parseAmount(partialPaymentAmount) || 0)}) = {formatPrice(parseAmount(remainingPaymentAmount) || 0)}
                  </p>
                </div>
              </div>
            </Card>
        ...
        {/* Deadline Date */}
        <div className="space-y-2">
          <Label htmlFor="edit-sale-deadline" className="text-xs sm:text-sm">{t('editSale.deadlineLabel')}</Label>
          <Input
            id="edit-sale-deadline"
            type="date"
            value={deadlineDate}
        ...
        {/* Notes */}
        <div className="space-y-2">
          <Label htmlFor="edit-sale-notes" className="text-xs sm:text-sm">{t('editSale.notesLabel')}</Label>
          <Textarea
            id="edit-sale-notes"
            value={notes}
